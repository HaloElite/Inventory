...
- **`components/InventoryList.vue:41`** — `mapCategoryNames` gibt bei Nichttreffer den String `'val'` zurück statt die Variable `val`.
- **`components/InventoryList.vue:169`** — die Mobile-Ansicht zeigt `item.category` roh („spare-parts" statt „Ersatzteile"), Desktop mappt korrekt.
- **`components/ModalEditItem.vue:59-60`** — die `InputCustom`-Felder bekommen kein `ar-label-name`, Screenreader lesen den Default `"label-custom"`.
- **`src/__tests__/App.spec.ts`** — noch das Vite-Template, erwartet „You did it!" und mountet ohne Router. Effektiv 0 Testabdeckung.
- **`components/Modal.vue:14`** — `watch` auf `open` ohne `immediate`: ein mit `open=true` gemountetes Modal öffnet nicht.
- **`components/InventoryList.vue:74`** — `@keydown.space` ohne `.prevent`, die Seite scrollt beim Aktivieren einer Zeile mit.
